    ...
    if ($show === true) {
        ...
        if ($layout == "1") {
            ...
            if (trim($course->image) == "") {
                $course->image = "components/com_guru/images/thumbs/no-image.jpg";
            ...
            if(trim($course->image) != ""){
                $image = '<img alt="Category Image" class="uk-width-1-1 '.$style_categs->ctgs_image.'" src="'.JURI::root().$course->image.'" />';
            ...
            $return .= '<div class="uk-card uk-card-body uk-card-default uk-padding-remove uk-border-rounded uk-overflow-hidden">';
            ...
            if ($img_align == 0) { // Centered image
                ...
                $return .= '<a class="galleryItem uk-inline uk-width-1-1" href="'.JRoute::_('index.php?option=com_guru&view=guruPcategs&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id)).'" title="'.$course->name.'">';
                $return .= '<span class="uk-position-top-right uk-label uk-label-danger uk-margin-small-top uk-margin-small-right uk-text-tiny font">'.$edit_sum.'</span>';
                if(trim($image) != ""){
                    $return .= $image;
                }
                $return .= '<div class="uk-overlay uk-position-bottom uk-padding-small"><p class="font uk-text-small">'.$course->name.'</p></div>';
                $return .= '</a>';
                ...
                $return .= '<div class="uk-width-1-1 uk-width-medium-2-3">';
                    ...
                    $return .= '<div class="'.$style_categs->ctgs_description.'"><p>'.$description.'</p>'.$rmore.'</div>';
...
function generateCoursesCellsB($config_courses, $style_courses, $course, $config) {
    ...
    if($layout == "1"){//mini profile
        ...
        if(trim($course->image_avatar) == ""){
            $course->image_avatar = "components/com_guru/images/thumbs/no-image.jpg";
        ...
        if (trim($course->image_avatar) != ""){
            ...
            $image = '<img class="uk-width-1-1" src="'.JURI::root().$course->image_avatar.'" />';
            ...
            $image_rounded = '<img src="'.JURI::root().$course->image_avatar.'" />';
        ...
        $return .= '<div class="guruBoxItem">';

        $return .= '<div>';

        $return .= '<div class="uk-card uk-card-body uk-card-default uk-padding-remove uk-border-rounded uk-overflow-hidden">';

        $return .= '<a class="galleryItem uk-inline uk-width-1-1" href="'.JRoute::_('index.php?option=com_guru&view=guruPrograms&task=view&cid='.$course->id."-".$alias."&Itemid=".intval($item_id)).'" title="'.$course->name.'">';
        ...
        elseif ($img_align == "1") {
            ...
            if($show_price == 0){
                $return .= '<span class="uk-position-top-right uk-label uk-label-danger uk-margin-small-top uk-margin-small-right uk-text-tiny font uk-text-capitalize">From '.$price.'</span>';
            }
            ...
            /*
            $return .=  '<ul><li>'.$authors_urls.'</li><li title="'.$count_courses_text_tip.'">'.$count_courses_text.'</li></ul>';
            $return .=  '<div>';
    ...
    if($categ->published == 1){
        ...
        if(trim($categ->image) != ""){
            if ($config_category->ctg_image_alignment == "1") {
                $categ_image = '<img border="0" alt="" class="" src="'.JURI::root().$categ->image.'" />';
            }
                    ...
                    $category_layout .= 
                       '<div>';
                            ...
                            if ($config_category->ctg_image_alignment == "0") { 
                                ...
                                if ($desc) {
                                    $category_layout .= '<div class="uk-card uk-card-body uk-card-default uk-padding-small uk-visible@m uk-border-rounded uk-margin-medium-bottom"><div class="uk-text-justify uk-text-small font uk-text-small">' . $desc . '</div></div>';
                                } else {
                                    $category_layout .= '';
                                }
                            }
                            ...
                            elseif ($config_category->ctg_image_alignment == "2") {
                                $category_layout .= '<div aaa>';
                                $category_layout .=   '<div bbb>';
                                $category_layout .=     '<h2>' . $categ->name . '</h2>';
                                $category_layout .=     '<div>'.$desc.'</div>';
                                $category_layout .=   '</div>';

                                if($categ_image != ""){
                                    $category_layout .= '<div ccc>'.$categ_image.'</div>';
                                }
                    ...
                    if(isset($subcateg) && count($subcateg) > 0){
                        ...
                        if($layout == "0"){
                            ...
                            $category_layout .= '<div class="cat_level_wrap cat_level_'.$var_inc.'" >
                                                        <div class="g_sect uk-child-width-1-1 uk-child-width-1-2@m uk-child-width-1-3@l" uk-grid>';
                            ...
                            */
                            ...
                            while(isset($categs_array[$i])){
                                $row = "";
//                                $row .= '<div class="uk-child-width-1-1 uk-child-width-1-2@m uk-child-width-1-3@l" uk-grid>';
                                ...
                                while($j < $cols){
                                    ...
                                    elseif(isset($categs_array[$i]) && trim($categs_array[$i]) != ""){  
                                        $row .= '<div><div class="guruBoxItem"><div>'.$categs_array[$i]."</div></div></div>";
                                        $j++;
                                    }
                                    $i++;
...
//                                $row .= '</div>';
                ...
                if($layout == "0"){
                    ...
                    /*
                    $category_layout .= '<div www>';
                    $category_layout .=     '<div class="uk-hidden"><input type="text" placeholder="'.JText::_("GURU_SEARCH").'" name="guru_search" value="'.trim($guru_search).'" class="form-control uk-margin-remove uk-form-width-small" />
                    <input type="submit" name="search_button" value="'.JText::_("GURU_SEARCH").'" class="uk-button uk-button-primary" style="margin:0px;">
                    </div>';
                    
                    if(count($courses_array) > 5){
                        $category_layout .= '<div eeee>'.$limit_pag.'</div>';
                    }
                    ...
                    */
                    ...
                    $category_layout .= '<div class="uk-child-width-1-1 uk-child-width-1-2@m uk-child-width-1-3@l" uk-grid>';
                    while(isset($courses_array[$i])){
                        $row = "";
                        for($j=0; $j<$cols; $j++){
                            if(count($courses_array) == 1){             
                                if(isset($courses_array[$i])){
                                    $row .= '<div xxx><div>'.$courses_array[$i++]."</div></div>";
                                }
                            }
                            else{
                                if(isset($courses_array[$i])){
                                        $row .= '<div lll"><div>'.$courses_array[$i++]."</div></div>";
                                }
                            }
                        }
                        $category_layout .= $row;
